...
func ChangeGrp(parametros ParametrosStructs.ParametrosChGrp) {
	fmt.Println("======Start CHGRP======")
	...
	if !existe_grupo {
		Structs.TextoEnviar.WriteString(fmt.Sprintf(" ❌ Error: El grupo no existe\n"))
		return
	}
	...
	for _, line := range lines {
		...
		if len(words) == 5 {
			if words[3] == user {
				...
				if words[0] != "0" {
					existe_usuario = true
					grupoModificado = words[0] + "," + words[1] + "," + grp + "," + words[3] + "," + words[4]
					nuevoData += grupoModificado + "\n"
					continue
				} else {
	...
	if usuario_eliminado {
		Structs.TextoEnviar.WriteString("❌ Error: El usuario ya fue eliminado\n")
		return
	}

	if !existe_usuario {
		Structs.TextoEnviar.WriteString("❌ Error: El usuario no existe\n")
		return
	}
	...
	Structs.TextoEnviar.WriteString(" ✅ Se cambió el grupo con exito\n")
