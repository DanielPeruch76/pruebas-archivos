...
func RMGrp(parametros ParametrosStructs.ParametrosRmGrp) {
	...
	for _, line := range lines {
		...
		if len(words) == 3 {
			if words[2] == name {
				...
				if words[0] != "0" {
					existe_grupo = true
					grupoModificado = "0," + words[1] + "," + words[2]
	...
	if grupo_eliminado {
		Structs.TextoEnviar.WriteString("❌ Error: El grupo ya fue eliminado previamente\n")
		return
	}

	if !existe_grupo {
		Structs.TextoEnviar.WriteString("❌ Error: El grupo que desea eliminar no existe\n")
		return
	}
	...
	Structs.TextoEnviar.WriteString(" ✅ Se eliminó grupo con exito\n")
